...
int main( int argc, char *argv[] ) {
  ...
  if ( myPart == CLIENT ) {
    ...
    case 2:
      ...
      break;
    case 3:
		for ( int windowSize = 1; windowSize <= MAXWIN; windowSize+= MAXWIN-1 ) {
			for ( loss = 0; loss <= 10; loss++) {
				timer.start( );                                        // start timer
				retransmits =
  ...
  if ( myPart == SERVER ) {
    ...
    case 3:
      for ( int windowSize = 1; windowSize <= MAXWIN; windowSize += MAXWIN-1 ) {
		for ( loss = 0; loss <= 10; loss++) {
		  serverEarlyRetrans( sock, MAX, message, windowSize, loss );
		}
	  }
      break;
    default:
